...
//import * as mathjs from "mathjs" DONT NEED THIS???
...
export default class PING_28015 extends Typings.Sensor {
   ...
   * 
   * @return Distance in millimeters to closet object to a resolution of 24 mm (too large)
   */
   ...
   *
   * @param pin Raspberry Pi B+ combine INPUT & OUTPUT pin name constant (i.e. GPIO1, GPIO2, GPIO3) 
   * @param direction Direction of ultrasonic audio pulse, used to calcalute distance
  ...
  private PollPING(pin = GPOI1, direction = PING_SENT, startTime = Date.now()): number{  
    if(direction == PING_SENT)
      return INVALID_STATE
    if(direction == PING_RECIEVED)
